...
type Step4Errors = Partial<Record<'phone_number' | 'email' | 'custom_message', string>>
...
export default function Step4() {
  ...
  const paymentDisabledReason = useMemo(() => {
    if (!store.couple_name_1 || !store.couple_name_2 || !store.wedding_date || !store.template_slug) {
      return 'Please complete Step 1 details.'
    }
    if (store.events.length === 0) return 'Please add at least one event in Step 2.'
    if (mediaCount === 0) return 'Please upload at least one media item in Step 3.'
    const check = step4Schema.safeParse({
      phone_number: store.phone_number,
      email: store.email,
      custom_message: store.custom_message,
    })
    if (!check.success) return 'Please complete contact details correctly before payment.'
    return null
  }, [mediaCount, store])
    ...
    <div className="space-y-8">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-[--color-charcoal]">Couple and template</h2>
          <Button size="sm" variant="ghost" onClick={() => store.goToStep(1)}>
            Edit
          </Button>
        </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-[--color-charcoal]">Events ({store.events.length})</h2>
          <Button size="sm" variant="ghost" onClick={() => store.goToStep(2)}>
            Edit
          </Button>
        </div>
        <div className="mt-3 space-y-2">
          {store.events.map((event, index) => (
            <div key={`${event.event_name}-${index}`} className="rounded-lg border border-gray-100 p-2.5 text-sm">
              <p className="font-medium text-[--color-charcoal]">{event.event_name}</p>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-[--color-charcoal]">Media ({mediaCount})</h2>
          <Button size="sm" variant="ghost" onClick={() => store.goToStep(3)}>
            Edit
          </Button>
        </div>
        <div className="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          ...
          {store.media.photos.length === 0 && (
            <p className="text-xs text-gray-400 col-span-full">No photo previews yet.</p>
          )}
        </div>
      </div>

      <div className="rounded-2xl border border-gray-200 bg-white p-4 space-y-4">
        <h2 className="text-base font-semibold text-[--color-charcoal]">Contact details</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Input
            ...
            onChange={(e) => store.updateContact({ phone_number: e.target.value })}
        ...
        <Input
          ...
          onChange={(e) => store.updateContact({ custom_message: e.target.value })}
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-semibold text-[--color-charcoal]">₹{PRICING.basePrice}</p>
          </div>
          <p className="text-xs text-gray-400 text-right">One-time payment<br />No hidden fees</p>
        </div>
        <div className="mt-4">
          <PaymentButton
            loading={isLoading}
