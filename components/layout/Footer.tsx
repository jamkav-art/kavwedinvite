...
export default function Footer() {
  return (
    <footer
      id="contact"
      className="bg-transparent text-stone-100 relative overflow-hidden"
    >
      {/* Decorative top gradient accent */}
      <div className="h-px bg-gradient-to-r from-transparent via-[--color-gold]/60 to-transparent" />

      {/* Cursor‑Reactive Ethereal Background */}
      ...
      <div className="bg-black/20 backdrop-blur-md rounded-2xl sm:rounded-3xl p-6 sm:p-8">
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
              ...
              <ul className="space-y-2.5">
                {[
                  ...
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-stone-100 hover:text-amber-200 transition-all duration-300 hover:-translate-y-1 hover:drop-shadow-lg"
                    >
                      {link.label}
                    </Link>
                  </li>
          ...
          {/* Bottom bar */}
          <div className="border-t border-stone-400/20 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-xs text-stone-100/50">
              © {new Date().getFullYear()} WedInviter. All rights reserved.
            </p>
            <div className="flex items-center gap-5">
              <Link
                href="/privacy"
                className="text-xs text-stone-100/50 hover:text-amber-200 transition-colors duration-200"
              >
                Privacy Policy
              </Link>
              <Link
