    ...
    async (orderData: OrderFormState) => {
      ...
      try {
        ...
        if (!createRes.ok || !createPayload.success || !createPayload.orderId) {
          throw new Error(createPayload.error ?? 'Failed to create payment order.')
        }
        ...
        const rzp = new window.Razorpay({
          key,
          amount: createPayload.amount ?? 69900,
          ...
          handler: async (paymentResponse) => {
            try {
              ...
              if (!verifyRes.ok || !verifyPayload.success || !verifyPayload.inviteId) {
